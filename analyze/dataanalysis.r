...
tim_c <- c("coldhalf") #all, winter, spring, summer, autumn, coldhalf, warmhalf
height_c <- c("0cm")
minmax_c <- c("max")
snow_categorical <- FALSE
...
ctrl = lmeControl(maxIter=100, msMaxIter=100, opt="optim")
all_loggers$curt_diff <- abs(all_loggers$curt_diff)
if (tim == "warmhalf"){
    ...
    mod_curt <- lme(curt_diff ~ snowcm + nt + hum + precmm + ffkmh + month,
        random = ~1 | station_name, data=all_loggers, na.action=na.exclude, method = "ML")
    mod_curt_corARMA <- update(mod_curt, correlation=corARMA(p=2, q=1))
}

#d_mod_curt <- data.frame(x=fitted(mod_curt_corARMA), y=resid(mod_curt_corARMA, type="n"))
#mod_resid_vs_fitted_and_qq(d_mod_curt, minmax, height, tim, "curt_abs")
setwd("/home/vojta/Desktop/mffuk/bakalarka/analyze/out/ARMAmods")
snow_text <- ifelse(snow_categorical == 1, "yes", "no")
abs_text <- ifelse(abs == TRUE, "abs_", "")
#save(file=paste("ARMAmod_abs_", minmax, height, tim, "_snowcategory_", snow_text, ".RData", sep=""), mod_curt_corARMA)
load(paste("ARMAmod_", minmax, height, tim, ".RData", sep=""))
load(paste("ARMAmod_", abs_text, minmax, height, tim, "_snowcategory_", snow_text, ".RData", sep=""))
